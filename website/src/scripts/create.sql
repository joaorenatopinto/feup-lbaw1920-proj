...
CREATE TYPE UserStatus AS ENUM ('active', 'moderator', 'banned', 'recoMod');

DROP TYPE IF EXISTS AuctionStatus;
CREATE TYPE AuctionStatus AS ENUM ('ongoing', 'removed', 'closed');

DROP TYPE IF EXISTS NotificationStatus;
CREATE TYPE NotificationStatus AS ENUM ('notSeen', 'seen');

DROP TYPE IF EXISTS ReportStatuss;
CREATE TYPE ReportStatus AS ENUM ('notSeen', 'seen', 'closed');
...
CREATE TABLE auctionStatus (
    id SERIAL PRIMARY KEY,
    TYPE AuctionStatus NOT NULL DEFAULT ongoing,
...
CREATE TABLE reportStatus (
    id SERIAL PRIMARY KEY,
    TYPE ReportStatus NOT NULL DEFAULT notSeen,
    dateChanged DATE DEFAULT now(),
    oldStatus VARCHAR
);
...
CREATE TABLE notificationStatus (
    id SERIAL PRIMARY KEY,
    TYPE NotificationStatus NOT NULL DEFAULT notSeen,
    dateChanged DATE DEFAULT now(),
);